...
def incrucisare(valori, afisare):
    ...
    for x in range(len(valori) // 2):
        ...
        if (afisare):
            date.outfile.write(f"Recombinare dintre cromozomul {idx_cromozomi_aleatori[0] + 1} si cromozomul {idx_cromozomi_aleatori[1] + 1}\n")
        ...
        if (afisare):
            date.outfile.write(f"{date.lista_cromozomi[idx_cromozomi_aleatori[0]]} {date.lista_cromozomi[idx_cromozomi_aleatori[1]]} punct {punct_crossover}\n")
